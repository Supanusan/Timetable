...
const Header = () => {
    ...
    return (
        <div className='flex flex-col items-center w-full h-[100px] py-6 px-3  '>
            ...
            <div>
                <button className='bg-orange-600 py-3 px-8 text-white text-xl rounded-md mt-[7vh]' onClick={FNavigation}>
                    To get Zoom Link
                </button>
                <div className=' absolute right-0 mt-6 w-[50%] hidden justify-evenly px-4 md:flex'>
                    {link.map((data, index) => (
                        <Link key={index} to={data.TO} className='linkstyle' smooth={true} spy={true} >{data.Name}</Link>
                    ))}
